...
institution1 = Institution.objects.create(name='Samotne matki', description='Pomaganie samotnym matkom w trudnej sytuacji materialnej.',type=Institution.FUNDACJA)
institution2 = Institution.objects.create(name='Trudne dzieci', description='Wyprowadzanie na prostą trudnej młodzieży',type=Institution.ORGANIZACJA_POZARZADOWA)
institution3 = Institution.objects.create(name='Drugie życie', description='Znajdowanie nowego właściciela zapomnianym przedmiotom',type=Institution.ZBIORKA_LOKALNA)
institution4 = Institution.objects.create(name='Defraudacja', description='Pomoc w prowadzeniu karuzel VAT-owskich.',type=Institution.FUNDACJA)
...
--
-- Donation.objects.get(pk=1).categories.add(Category.objects.get(name='Różne'))
